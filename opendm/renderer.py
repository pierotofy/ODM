        ...
        for mat in materials_idx:
            ...
            if mat == materials_idx[0]:
                ...
                for n in range(texture.shape[2]):
                    band = np.full((width, height), fill_value, dtype=data_type)
                    bands.append(band)
            ...
            rows, cols = map(float, texture.shape[:2])
            num_channels = texture.shape[2]
            ...
            for f in obj['faces'][mat]:
                ...
                if is_sliver_polygon(v1, v2, v3):
                    _warn("Sliver polygon found at %s" % f)
                    continue
                ...
                minx = int(min((v1[0], v2[0], v3[0])))
                if minx > width:
                    continue # Completely outside to the right
                maxx = int(max((v1[0], v2[0], v3[0])))
                if maxx < 0:
                    continue # Completely outside to the left
                miny = int(min((v1[1], v2[1], v3[1])))
                if miny > height:
                    continue # Completely outside to the top.
                maxy = int(max((v1[1], v2[1], v3[1])))
                if maxy < 0:
                    continue # Completely outside to the bottom.
                    ...
                    for rq in range(rq_start, rq_end):
                        ...
                        for cq in range(cq_start, cq_end):
                            ...
                            for i in range(0, num_channels):
                                ...
                                tl = float(texture[top][left][i])
                                tr = float(texture[top][left+1][i])
                                bl = float(texture[top+1][left][i])
                                br = float(texture[top+1][left+1][i])
